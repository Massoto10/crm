...
export default function AgendamentosPage() {
  ...
  return (
    <RoleGuard allowed={['institution_admin', 'operator']}>
      <PageHeader title="Agendamentos" subtitle="Mensagens programadas diretamente na sua operação." actions={<><div className="segmented-actions"><button className={view === 'list' ? 'active' : ''} onClick={() => setView('list')}><List size={16} /> Lista</button><button className={view === 'calendar' ? 'active' : ''} onClick={() => setView('calendar')}><CalendarDays size={16} /> Agenda</button></div><Button onClick={() => setOpen(true)}><Plus size={17} /> Novo agendamento</Button></>} />
