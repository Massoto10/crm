...
export function ConfiguracoesClientPage() {
    ...
    <RoleGuard allowed={['institution_admin']}>
      ...
      <div className="settings-layout">
        <Card className="settings-nav">
          {tabs.map(({ id, label, icon: Icon }) => <button key={id} className={tab === id ? 'active' : ''} onClick={() => setTab(id)}><Icon size={18} /><span>{label}</span></button>)}
        </Card>
