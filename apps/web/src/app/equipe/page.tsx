...
export default function EquipePage() {
  ...
  return (
    <RoleGuard allowed={['institution_admin']}>
      <PageHeader title="Equipe" subtitle="Operadores e departamentos carregados da API da instituição." actions={<Button onClick={() => setOpen(true)}><Plus size={17} /> Novo operador</Button>} />
      <div className="stats-grid stats-grid-3">
        ...
        <StatCard label="Departamentos" value={departments.length} helper="Estrutura atual da equipe" icon={<Users size={22} />} tone="purple" />
      </div>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="Novo operador" footer={<><Button variant="secondary" onClick={() => setOpen(false)}>Cancelar</Button><Button form="operator-form" type="submit" disabled={saving}>{saving ? 'Criando...' : 'Criar operador'}</Button></>}>
        <form id="operator-form" className="form-grid" onSubmit={create}>
          ...
          <label className="full"><span>Departamento</span><select value={form.departmentId} onChange={(event) => setForm({ ...form, departmentId: event.target.value })}><option value="">Sem departamento</option>{departments.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
        </form>
      </Modal>
    </RoleGuard>
  );
